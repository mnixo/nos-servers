...
</custom-style>`;template$8.setAttribute("style","display: none;");document.head.appendChild(template$8.content);const $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML=`<dom-module id="paper-dialog-shared-styles">
  <template>
    <style>
      :host {
        display: block;
...
`;template$9.setAttribute("style","display: none;");document.head.appendChild(template$9.content);Polymer({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
</dom-module>`;document.head.appendChild($_documentContainer$3.content);Polymer({_template:html`
...
</dom-module>`;document.head.appendChild($_documentContainer$4.content);const template$11=html`
...
`;template$11.setAttribute("strip-whitespace","");Polymer({_template:template$11,is:"paper-spinner",behaviors:[PaperSpinnerBehavior]});const sendEvent=(action,details)=>{if("undefined"!==typeof gtag){gtag("event",action,details)}};_exports.sendEvent=sendEvent;_exports.$analytics={sendEvent:sendEvent};const defaultConfig=[{label:"Production",url:"https://connect.nuxeo.com",studioProjects:["nuxeo-connect","nuxeo-university","nuxeo-sprint-report"]},{label:"Pre-Production",url:"https://nos-preprod-connect.nuxeocloud.com",studioProjects:["nuxeo-connect","nuxeo-university","nuxeo-sprint-report"]}];_exports.defaultConfig=defaultConfig;_exports.$defaultConfig={defaultConfig:defaultConfig};class NOSBaseClass extends LitElement{render(){return html$1``}getById(id){return this.shadowRoot.getElementById(id)}fire(eventName,detail){this.dispatchEvent(new CustomEvent(eventName,{detail}))}}_exports.NOSBaseClass=NOSBaseClass;window.customElements.define("nos-base-class",NOSBaseClass);_exports.$nosBaseClass={NOSBaseClass:NOSBaseClass};class NOSServerDialog extends NOSBaseClass{static get properties(){return{server:{type:Object}}}constructor(){super();this.server={}}render(){return html$1`
      <style>
        paper-dialog {
          display: flex;
          flex-direction: column;
    ...
    `}_renderStudioButton(server){const hasProjects=server&&server.studioProjects&&0<server.studioProjects.length;return html$1`
      <paper-button raised dialog-dismiss @tap="${this._onStudioTap.bind(this)}" .disabled="${!hasProjects}">
        studio
      </paper-button>
    `}open(server){this.server=server;this.getById("dialog").open()}_openNewTab(url){window.open(url,"_blank").focus()}_onConnectDashboardTap(){sendEvent("connect",{});this._openNewTab(this.server.url)}_onNuxeoBackendTap(){sendEvent("backend",{});this._openNewTab(`${this.server.url}/nuxeo`)}_onStudioTap(){this.fire("studio-tap",{server:this.server})}}window.customElements.define("nos-server-dialog",NOSServerDialog);class NOSServerStatus extends NOSBaseClass{static get properties(){return{url:{type:String},_status:{type:String},_message:{type:String}}}render(){return html$1`
      <style>
        :host {
          display: flex;
          align-items: center;
      ...
      ${this._renderIcon()}
      <p>${this._message}</p>
    `}updated(changedProperties){if(changedProperties.has("url")){this._pingServer()}}_pingServer(){this._status="?";this._message="Waiting...";const timeBefore=new Date;fetch(new Request(`${this.url}/nuxeo/site/connect/dashboard`,{mode:"no-cors"})).then(()=>{this._status="ok";this._message=`Online (${new Date-timeBefore}ms)`}).catch(()=>{this._status="ko";this._message=`Offline (${new Date-timeBefore}ms)`})}_renderIcon(){if("?"===this._status){return html$1`
        <paper-spinner active></paper-spinner>
      `}const style="ok"===this._status?"ok":"ko",icon="ok"===this._status?"check":"close";return html$1`
      <iron-icon class="${style}" icon="${icon}"></iron-icon>
    `}}window.customElements.define("nos-server-status",NOSServerStatus);class NOSServerStudioDialog extends NOSBaseClass{static get properties(){return{server:{type:Object}}}constructor(){super();this.server={}}render(){return html$1`
      <style>
        paper-dialog {
          display: flex;
          flex-direction: column;
    ...
    `}_renderProjectOptions(server){if(!server||!server.studioProjects||0===server.studioProjects.length){return}return server.studioProjects.map(studioProject=>html$1`
      <paper-item .value="${studioProject}">${studioProject}</paper-item>
    `)}open(server){this.server=server;this.getById("listbox").selected=0;this.getById("dialog").open()}_openNewTab(url){window.open(url,"_blank").focus()}_onModelerTap(){sendEvent("studio-modeler",{});this._openNewTab(`${this.server.url}/nuxeo/site/studio/ide?project=${this.getById("listbox").selectedItem.value}`)}_onDesignerTap(){sendEvent("studio-designer",{});this._openNewTab(`${this.server.url}/nuxeo/designer/#/${this.getById("listbox").selectedItem.value}`)}}window.customElements.define("nos-server-studio-dialog",NOSServerStudioDialog);class NOSServer extends NOSBaseClass{static get properties(){return{server:{type:Object}}}constructor(){super();this.server={}}render(){return html$1`
      <style>
        paper-card {
          width: 100%;
          margin-bottom: 0.5em;
    ...
    `}_onTap(){this.fire("tap")}}window.customElements.define("nos-server",NOSServer);class NOSServersConfigDialog extends NOSBaseClass{static get properties(){return{isValid:{type:Boolean}}}constructor(){super();this.isValid=!1}render(){return html$1`
      <style>
        paper-dialog {
          width: 100%;
          max-width: 800px;
    ...
    `}open(config){this.getById("textArea").value=JSON.stringify(config,null,2);this.getById("dialog").open()}_onValueChanged(){this.getById("dialog").center();try{JSON.parse(this.getById("textArea").value);this.isValid=!0}catch(error){this.isValid=!1}}_onReset(){this.getById("textArea").value=JSON.stringify(defaultConfig,null,2)}_onSubmitConfig(){this.fire("config-submitted",{config:JSON.parse(this.getById("textArea").value)})}}window.customElements.define("nos-servers-config-dialog",NOSServersConfigDialog);class NOSServersApp extends NOSBaseClass{static get properties(){return{_servers:{type:Array}}}constructor(){super();this._servers=JSON.parse(localStorage.getItem("servers"));if(!this._servers){this._servers=defaultConfig;localStorage.setItem("servers",JSON.stringify(this._servers))}}render(){return html$1`
      <style> 
        app-header {
      ...
      <app-header reveals>
        <app-toolbar>
          <paper-icon-button src="img/icon-32.png"></paper-icon-button>
    ...
    `}_renderServers(servers){return servers.map(server=>html$1`
      <nos-server .server="${server}" @tap="${()=>this._onServerTap(server)}"></nos-server>
    `)}_openConfigDialog(){this.getById("configDialog").open(this._servers)}_onConfigSubmitted(e){this._servers=e.detail.config;localStorage.setItem("servers",JSON.stringify(this._servers))}_onServerTap(server){this.getById("serverDialog").open(server)}_onGoToStudioTap(e){this.getById("studioDialog").open(e.detail.server)}}window.customElements.define("nos-servers-app",NOSServersApp)});